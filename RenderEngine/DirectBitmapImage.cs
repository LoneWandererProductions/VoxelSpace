...
public sealed class DirectBitmapImage : IDisposable
{
    ...
    public void FillSimd(Color color)
    {
        ...
        var i = 0;
        for (; i <= Bits.Length - Vector<uint>.Count; i += Vector<uint>.Count) vector.CopyTo(Bits, i);

        for (; i < Bits.Length; i++) Bits[i] = packed;
    ...
    public void ApplyColorMatrix(float[][] matrix)
    {
        ...
        for (var i = 0; i < Bits.Length; i++)
        {
            ...
            {
                result[j] = 0;
                for (var k = 0; k < 4; k++) result[j] += matrix[j][k] * input[k];
            }
    ...
    private void Dispose(bool disposing)
    {
        ...
        if (disposing && _bitsHandle.IsAllocated) _bitsHandle.Free();
